...
def edit(slug = None):
  ...
  return render_template('edit.html', form = form, slug=slug, cheat_id = cheat.id)


@admin.route('/delete/<string:slug>/', methods=['GET', 'POST'])
def delete(slug = None):
  d = session.execute(Cheat.__table__.delete().where(Cheat.__table__.c.slug == slug))
  if d.rowcount:
    session.commit()
  else: 
    # just a stab if we will want to say something if the rows wasn't existed
    # probably, we could've drop this request with 404 or email warning, etc.
    pass
